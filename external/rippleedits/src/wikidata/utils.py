...
with zipfile.ZipFile('./external/rippleedits/src/wikidata/ent_label2id.json.zip', 'r') as zip_ref:
    zip_ref.extractall('./external/rippleedits/src/wikidata/ent_label2id.json')

ent_label2id_dict = load_json('./external/rippleedits/src/wikidata/ent_label2id.json/ent_label2id.json')
ent_id2label_dict = {qid: label for label, qid in ent_label2id_dict.items()}
...
def get_label(ent_id: str):
    ...
    if ent_id[0] != 'Q':
        return ent_id
    label = ent_id2label_dict.get(ent_id)
    if label: 
        return label
    ...
    try:
        r = requests.get(
            ...
            headers={"User-Agent": "label-lookup/1.0 ()"},
            timeout=10,
        )
        if not r.ok:
            return ent_id
        data = r.json() or {}
        ent = data.get("entities", {}).get(ent_id)
        if not ent:
            return ent_id
...
def get_aliases(ent_id: str):
    try:
        r = requests.get(
            ...
            headers={"User-Agent": "alias-lookup/1.0 ()"},
            timeout=10,
        )
        if not r.ok:
            return []
        ...
        data = r.json() or {}
        ent = (data.get("entities") or {}).get(ent_id) or {}
        aliases = ent.get("aliases") or {}

        if "en" in aliases and isinstance(aliases["en"], list):
            return [a.get("value") for a in aliases["en"] if isinstance(a, dict) and "value" in a]
